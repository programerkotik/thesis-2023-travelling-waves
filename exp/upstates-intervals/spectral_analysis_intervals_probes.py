# Analyse spectral properties of upstate and downstate intervals in Probes data.
...
for file in files:
    if file.endswith('.npy'):
        # if file == 'ecog.npy':
        #     ecog_data = np.load(input_dir + '/' + file, allow_pickle=True) # no analysis of ecog here
        if file == 'probe1.npy':
...
data = [probe1_data, probe2_data]
names = ['Probe_1', 'Probe_2']

for d, name in zip(data,names):
    # Split data into upstate and downstate intervals
    probe_upstates, probe_downstates = split_intervals(d, times, upstates, downstates)
    ...
    pd.DataFrame(spectral_properties).to_csv(f"{output_dir}/spectral_properties_{name}.csv", index=False, float_format='%.3f')
